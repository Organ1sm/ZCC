...
pub fn RegisterManager(
    ...
    comptime calleePreservedRegs: []const Register,
) type {
    return struct {
        /// The key must be canonical register.
        registers: [calleePreservedRegs.len]?Index = [_]?Index{null} ** calleePreservedRegs.len,
        freeRegisters: FreeRegInt = math.maxInt(FreeRegInt),
        /// Tracks all registers allocated in the course of this function
        allocatedRegisters: FreeRegInt = 0,

        const Self = @This();

        /// An integer whose bits represent all the registers and whether they are free.
        const FreeRegInt = std.meta.Int(.unsigned, calleePreservedRegs.len);
        ...
        fn markRegUsed(self: *Self, reg: Register) void {
            ...
            const shift = @as(ShiftInt, @intCast(index));
            const mask = @as(FreeRegInt, 1) << shift;
            self.freeRegisters &= ~mask;
            self.allocatedRegisters |= mask;
        }
        ...
        fn markRegFree(self: *Self, reg: Register) void {
            ...
            const index = reg.allocIndex() orelse return;
            const shift = @as(ShiftInt, @intCast(index));
            self.freeRegisters |= @as(FreeRegInt, 1) << shift;
        }
        ...
        pub fn isRegFree(self: Self, reg: Register) bool {
            ...
            const shift = @as(ShiftInt, @intCast(index));
            return self.freeRegisters & @as(FreeRegInt, 1) << shift != 0;
        }
        ...
        pub fn isRegAllocated(self: Self, reg: Register) bool {
            ...
            const shift = @as(ShiftInt, @intCast(index));
            return self.allocatedRegisters & @as(FreeRegInt, 1) << shift != 0;
        }
        ...
        pub fn tryAllocRegs(
            ...
            comptime if (calleePreservedRegs.len == 0) return null;
