...
const DafsaBuilder = struct {
    ...
    fn writeDafsa(db: *const DafsaBuilder, writer: *std.Io.Writer) !void {
        ...
        var childIndexes: std.AutoHashMapUnmanaged(*Node, usize) = .empty;
        defer childIndexes.deinit(db.allocator);

        try childIndexes.ensureTotalCapacity(db.allocator, @intCast(db.edgeCount()));

        var firstAvalilableIndex: usize = db.root.numDirectChildren() + 1;
        firstAvalilableIndex = try db.writeDafsaChildren(db.root, writer, &queue, &childIndexes, firstAvalilableIndex);

        while (queue.items.len > 0) {
            // TODO: something with better time complexity
            const node = queue.orderedRemove(0);

            firstAvalilableIndex = try db.writeDafsaChildren(node, writer, &queue, &childIndexes, firstAvalilableIndex);
        }
    ...
    fn writeDafsaChildren(
        ...
        childIndexes: *std.AutoHashMapUnmanaged(*Node, usize),
