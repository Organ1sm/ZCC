...
/// A `ExpandedLocation` is a `Location` expanded with the text of the line and its width.
///
/// `path` is the path of the source file.
/// `line` is the text of the line.
/// `lineNo` is the line number (1-indexed).
/// `col` is the column number (1-indexed).
/// `width` is the width of the text of the line in columns.
/// `endWithSplice` is a boolean indicating if the line ends with a splice.
