...
fn findPaths(self: *Linux, tc: *Toolchain) !void {
    ...
    const multiarchTriple = getMultiarchTriple(target) orelse target.toLLVMTriple(&output);
...
pub fn buildLinkerArgs(self: *const Linux, tc: *const Toolchain, argv: *std.ArrayList([]const u8)) Compilation.Error!void {
    ...
    if (target.ldEmulationOption(null)) |emulation| {
...
test Linux {
    ...
    var testingIoVtable = std.testing.io.vtable.*;
    testingIoVtable.dirAccess = fakeFns.dirAccess;
    testingIoVtable.dirOpenFile = fakeFns.dirOpenFile;
    testingIoVtable.fileClose = fakeFns.fileClose;
    testingIoVtable.fileReadPositional = fakeFns.fileReadPositional;
    const fakeIo: std.Io = .{
        .userdata = std.testing.io.userdata,
        .vtable = &testingIoVtable,
    };

    var comp = Compilation.init(gpa, arena, fakeIo, undefined, std.fs.cwd());
    defer comp.deinit();
    ...
    comp.target = .fromZigTarget(try std.zig.system.resolveTargetQuery(fakeIo, targetQuery));
