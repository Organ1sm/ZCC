...
test "minUnsignedBits" {
    ...
    comp.target = .fromZigTarget(try std.zig.system.resolveTargetQuery(io, targetQuery));
...
pub fn toInt(v: Value, comptime T: type, comp: *const Compilation) ?T {
    ...
    return bigInt.toInt(T) catch null;
}
...
pub fn print(
    ...
    comp: *const Compilation,
    w: *std.Io.Writer,
) std.Io.Writer.Error!?NestedPrint {
    if (qt.is(comp, .bool)) {
...
pub fn printString(
    ...
    try w.writeByte('"');
    switch (size) {
        .@"1" => try std.zig.stringEscape(withoutNull, w),
        .@"2" => {
